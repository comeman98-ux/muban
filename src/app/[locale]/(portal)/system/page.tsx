...
export default function InternalSystemPage() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
    try {
      // 从 document.cookie 里找到我们这颗 session cookie
      const cookiePair = document.cookie
        .split("; ")
        .find((row) =>
          row.startsWith(`${SYSTEM_SESSION_COOKIE_NAME}=`)
        );
      ...
      // 用刚才在 lib 里写好的工具函数解析
      const user = makeCurrentUser(rawValue);
  ...
  useEffect(() => {
    const loadIdentities = async () => {
      if (!supabase) {
        console.error(
          "[system/page] Supabase client is not configured. Please set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY.",
        );
        return;
      }
      ...
      const { data, error } = await supabase
        .from("system_users")
        .select(
          "id, identity_id, is_active, can_access_docs, can_access_courses, is_admin, created_at, last_login_at",
        )
        .order("id", { ascending: true });

      if (error) {
        console.error("[system/page] Failed to load system_users:", error);
        return;
      }

      const rows: IdentityRecord[] =
        (data ?? []).map((user: any) => ({
          id: String(user.identity_id ?? ""),
          note: "",
          role: "",
          enabled: Boolean(user.is_active),
          canAccessDocs: Boolean(user.can_access_docs),
          canAccessCourses: Boolean(user.can_access_courses),
  ...
  const handleIdentityManageClick = () => {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          ...
          <div className="flex gap-3 md:justify-end">
            <button
              type="button"
              onClick={handleIdentityManageClick}
              className={[
